...
# {{ trans('mail.user_agreement_signature_reminder_greeting', ['name' => $agreement->user?->first_name ?? '']) }}

{{ trans('mail.user_agreement_signature_reminder_body', ['type' => $typeLabel, 'asset_tag' => $variables['asset_tag'] ?? '—', 'model' => $variables['model'] ?? '—', 'number' => $reminderNumber]) }}
...
{{ trans('mail.user_agreement_signature_reminder_cta') }}
</x-mail::button>
