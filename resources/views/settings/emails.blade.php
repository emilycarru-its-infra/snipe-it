        ...
        <div class="col-md-8">
            <div class="panel box box-default">
                ...
                <div class="box-body">
                    ...
                    {{-- The preview is injected via srcdoc rather than navigated to
                         with src, so the frame is not a same-origin navigation and
                         X-Frame-Options: DENY can stay in force everywhere. sandbox
                         is left empty on purpose: the rendered email is inert, so no
                         script or form in a template can do anything here. --}}
    ...
    (function () {
        ...
        function select(el) {
            ...
            if (previewable) {
                ...
                // Fetch the rendered email and hand it to the frame as srcdoc.
                // A plain src= would be a framed navigation, which
                // X-Frame-Options: DENY correctly blocks.
                frame.srcdoc = '';
                fetch(url, { credentials: 'same-origin' })
                    .then(function (r) { return r.ok ? r.text() : Promise.reject(r.status); })
                    .then(function (html) { frame.srcdoc = html; })
                    .catch(function () {
                        frame.srcdoc = '<p style="font-family:sans-serif;padding:1em;">'
                            + {!! json_encode(trans('admin/settings/general.emails_preview_error')) !!}
                            + '</p>';
                    });
                openTab.href = url;
