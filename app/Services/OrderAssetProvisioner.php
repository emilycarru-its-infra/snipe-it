...
class OrderAssetProvisioner
{
    ...
    private function provisionUnit(Order $order, OrderItem $line, int $modelId, int $statusId, ?string $poNumber): ?Asset
    {
        ...
        // Written as a raw attribute in Y-m-d, because that is the shape
        // the asset rules validate against (date_format:Y-m-d) and the
        // property is annotated as a Carbon. Assigning the order's cast
        // Carbon straight across stringifies with a time, fails validation,
        // and — since provisioning logs rather than throws — the order
        // silently provisioned nothing at all.
        $asset->setAttribute('purchase_date', $order->order_date?->format('Y-m-d'));
